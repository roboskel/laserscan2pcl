...
roboskel_msgs::LaserScans laserscans;

laser_geometry::LaserProjection projector;

tf::TransformListener* tfListener;

roboskel_msgs::PointCloud2_Segments bufferToAccumulator(const sensor_msgs::LaserScan::ConstPtr scan_in){
    ...
    roboskel_msgs::PointCloud2_Segments c;
...
void scanCallback (const sensor_msgs::LaserScan::ConstPtr scan_in){
    if (! ualv) {
        ...
        laserscans.header.stamp = ros::Time::now();
...
int main(int argc, char** argv){
    ...
    n.param("laserscan_stacker/use_a_laserscan_vector_instead", ualv, false);

    ros::Subscriber s = n.subscribe(input_topic, 1, scanCallback);
    if (!ualv){
        pub = n.advertise<roboskel_msgs::PointCloud2_Segments> (out_topic, 1);
    }
    else{
        pub = n.advertise<roboskel_msgs::LaserScans> (out_topic, 1);
    }
